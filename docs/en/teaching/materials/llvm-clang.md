...
The `Target:` line specifies the default target triple containing the processor architecture, the processor sub-architecture (optional), the processor vendor, the operating system, the environment, and the object format (optional). In the absence of additional parameters spoecifying the architecture, we will compile the code for the host processor architecture ([x86-64](https://en.wikipedia.org/wiki/X86-64) in our case, also known as [AMD64](https://docs.amd.com/v/u/en-US/40332_4.09_APM_PUB) and [Intel EM64T](https://www.intel.com/content/www/us/en/support/articles/000005898/processors.html)), the host operating system (Linux in our case), and the host environment (GNU). Notice how the host processor vendor is unknown, as Clang compiled for x86-64 can be executed on both Intel and AMD processors.
...
On Linux, Clang will by default link to [the GNU C++ library (libstdc++)](https://gcc.gnu.org/onlinedocs/libstdc++/) if one is present on the system. If multiple versions of GCC (and respective libstdc++) are present on the system, Clang will use the newest one it finds, but the selection can be overriden by specifying the `--gcc-toolchain=` parameter, e.g. `--gcc-toolchain=/opt/gcc-15.2.0`, or `--gcc-install-dir=` parameter, e.g. `--gcc-install-dir=/opt/gcc-15.2.0/lib/gcc/x86_64-pc-linux-gnu/15.2.0`. In the first case, if the machine-vendor-operatingsystem [target triple](https://wiki.osdev.org/Target_Triplet) is different from the one that Clang was built with, it can be set manually using `--gcc-triple=` parameter, e.g. `--gcc-triple=x86_64-pc-linux-gnu`.

On other operating systems, such as [Free](https://wiki.freebsd.org/NewC++Stack)[BSD](https://freebsdfoundation.org/wp-content/uploads/2021/05/FreeBSD-13.0-Tool-Chain.pdf) and [macOS](https://developer.apple.com/xcode/cpp/), linking to libc++ is the default. This can also be done on Linux using the `-stdlib=libc++` parameter.
