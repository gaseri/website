# Programiranje za web

## Predavanja

- [Uvodno predavanje](../materijali/web-uvod.md)
- [Arhitekture web aplikacija: monolitna i uslužno orijentirana](../materijali/web-arhitekture.md)
- [Paradigme programiranja u web aplikacijama: proceduralna, objektno orijentirana, funkcijska](../materijali/web-paradigme-programiranja.md)
- [Objektno modeliranje i programiranje web aplikacija. Objektno-relacijsko preslikavanje](../materijali/web-objektno.md)
- [Uzorci dizajna u web aplikacijama. Uzorci model-pogled-*](../materijali/web-uzorci-dizajna.md)
- [Faktori razvoja web aplikacija](../materijali/web-faktori.md)
- [Pisanje i provođenje automatiziranih testova programskog koda i web aplikacija](../materijali/web-testiranje.md)
- [Statička analiza programskog koda web aplikacija](../materijali/web-staticka-analiza.md)
- [Pisanje dokumentacije programskog koda i web aplikacija](../materijali/web-dokumentiranje.md)
- [Timski razvoj web aplikacija i upravljanje verzijama programskog koda](../materijali/web-verzioniranje.md)
- [Kontinuirana integracija i isporuka web aplikacija](../materijali/web-kontinuirana-integracija.md)
- [Kontejnerizacija web aplikacija](../materijali/web-kontejnerizacija.md)
- [Podrška web aplikacija za izvođenje na različitim verzijama platforme](../materijali/web-verzije-platforme.md)
- [Mjerenje i tehnike poboljšanja performansi web aplikacija](../materijali/web-performanse.md)
- [Sigurnost web aplikacija](../materijali/web-sigurnost.md)
- [Prošlost, sadašnjost i budućnost weba kao platforme za razvoj aplikacija i usluga](../materijali/web-evolucija.md)

## Vježbe

### Ponavljanje gradiva i temeljni koncepti

- [Rad s protokolima aplikacijske razine](../materijali/curl-protokoli-aplikacijske-razine.md)
- [HTTP klijent HTTPie](../materijali/httpie-http-klijent.md)
- [Ugrađeni web poslužitelj u interpreteru jezika PHP](../materijali/php-ugradjeni-web-posluzitelj.md)
- [Obrada HTTP zahtjeva i stvaranje odgovora u jeziku PHP](../materijali/php-zahtjev-odgovor.md)
- [Slanje podataka u HTTP zahtjevu metodama GET i POST u jeziku PHP](../materijali/php-slanje-podataka.md)
- [Postavljanje datoteka na poslužitelj u jeziku PHP](../materijali/php-file-upload.md)
- [Višejezičnost u jeziku PHP](../materijali/php-visejezicnost.md)
- [Kodiranje HTTP sadržaja i kompresija u jeziku PHP](../materijali/php-http-encoding.md)
- [HTTP autentifikacija u jeziku PHP](../materijali/php-http-auth.md)
- [Obrada podataka zapisanih u obliku JavaScript Object Notation (JSON) u jeziku PHP](../materijali/php-json.md)
- [Baratanje HTTP kolačićima u jeziku PHP](../materijali/php-cookies.md)
- [Implementacija REpresentational State Transfer (REST) aplikacijskog programskog sučelja u jeziku PHP](../materijali/php-rest.md)

### Izrada web aplikacija korištenjem Python okvira Django

- [Python: Django](../materijali/python-modul-django.md)
- [Python: Django REST framework](../materijali/python-modul-django-rest-framework.md)
